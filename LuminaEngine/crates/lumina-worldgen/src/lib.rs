...
// числом без остатка, и fract(v) всегда даёт 0. Считаем хэш в f64 (там
// порог точности — 2^52), чтобы дробная часть реально сохранялась.
//
// ВАЖНО #2 (реальный баг, из-за которого игрок всегда спавнился в воде):
// seed раньше был множителем РЕЗУЛЬТАТА (`* (seed + 1.0)`), а не сдвигом
// сэмплируемых координат. В точке (x=0, y=0) — а это буквально угол
// чанка (0,0), рядом с которым всегда стоит спавн (8,8) — sin(0*A+0*B) =
// sin(0) = 0 АБСОЛЮТНО ДЛЯ ЛЮБОГО seed, потому что 0 умножить на что
// угодно даёт 0 ещё до применения seed. В итоге рельеф у спавна был почти
// одинаковым (высота ~30-36, ниже уровня моря 45) во всех мирах, какой бы
// seed ни выбрали. Теперь seed сдвигает сами координаты сэмплирования —
// разные миры реально сэмплируют разные, никак не связанные с (0,0)
// участки шума, и вырожденная точка (0,0) больше не совпадает с точкой,
// одинаковой для всех seed.
fn seed_offset(seed: f64) -> f64 {
    seed % 100_000.0 // ограничиваем аргумент sin(), чтобы не терять точность на огромных seed
}

fn hash(x: f64, y: f64, seed: f64) -> f64 {
    let s = seed_offset(seed);
    let sx = x + s * 127.1 + 311.7;
    let sy = y + s * 269.5 + 183.3;
    let v = (sx * 12.9898 + sy * 78.233).sin() * 43758.5453123;
    v - v.floor() // fract()
}
...
fn hash3d(x: f64, y: f64, z: f64, seed: f64) -> f64 {
    ...
    let sx = x + s * 127.1 + 311.7;
    let sy = y + s * 269.5 + 183.3;
    let sz = z + s * 419.2 + 271.9;
    let v = (sx * 12.9898 + sy * 78.233 + sz * 37.719).sin() * 43758.5453123;
    v - v.floor()
}
